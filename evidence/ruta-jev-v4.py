...
GUIA = {
    ...
    "research": ("[ruta: research] Paso 1: carga el skill 'wiki-llm-rosero' con skill_view y "
                 "consulta el wiki local: usa search_files sobre ~/wiki y read_file sobre "
                 "index.md y las paginas que encuentres (es una carpeta de markdown en esta "
                 "maquina, NO es internet). Paso 2: si el wiki no tiene datos suficientes, "
                 "entonces usa web_search y web_extract. No respondas de memoria."),
    "memory": ("[ruta: memory] Paso 1: busca en fact_store con fact_store(action='search') "
               "- es una tool directa, no requiere cargar ningun skill. Paso 2: si no esta ahi, "
               "dilo y ofreceme las vias para buscarlo (wiki ~/wiki, documentos, o web) y "
               "pregunta antes de salir. Paso 3: con lo que hayas obtenido de fact_store, "
               "resuelve con libertad, como en others."),
    ...
    "user": ("[ruta: user] Paso 1: carga el skill 'wiki-llm-rosero' con skill_view y revisa "
              "las fuentes primarias del usuario: lee ~/.hermes/memories/USER.md con read_file "
              "y consulta el wiki local con search_files sobre ~/wiki (carpeta markdown en esta "
              "maquina, NO internet). Paso 2: si no esta en esas fuentes, usa la tool clarify "
              "para PEDIR APROBACION antes de usar web_search."),
